...
function RequirementTranscript({
  ...
  return (
    <ChatMessageList density="compact" gap={2} isStreaming={running}>
      ...
      {notices.length > 0 ? (
        <Banner status="info" title={notices.at(-1)?.message ?? ""} />
      ) : null}
...
function RequirementPromptLayer({ children }: { children: ReactNode }) {
...
function RequirementAskCard({
  ...
  if (!question || !answer) {
    return (
      <Card className="rq-shelf" padding={4}>
        <ChatSystemMessage>暂无需要澄清的问题</ChatSystemMessage>
      </Card>
    );
  }
    ...
    <Card className="rq-shelf" padding={4}>
      <VStack gap={3} align="stretch">
        <HStack justify="between" align="center">
          ...
          <Text type="supporting" size="2xs">
            {safeIndex + 1}/{prompt.questions.length}
          </Text>
        </HStack>
    ...
    <Card className="rq-shelf" padding={4}>
      <VStack gap={3} align="stretch">
        ...
        <List listStyle="disc" density="compact">
          {prompt.draft.acceptance_criteria.map((item, index) => (
            <ListItem
              key={`${item}-${index}`}
              label={
                <Text type="body" size="2xs" color="secondary">
