...
一个 WorkflowRun，并在受管 integration worktree 中执行。合法的同层并行组会分配独立
item worktree 真并发；其他工作项具有明确串行依赖。失败 WorkflowRun 会暂停
...
- `POST /api/requirements/{id}/workflow-run`：为已确认需求生成 WorkPlan，并启动或续跑
  该需求唯一的 WorkflowRun。若失败原因是 ChangeSpec 证据无效，会先恢复需求 session 修正
  `message-N` 与逐字原文；仅证据字段变化时自动继续，否则重新展示确认卡
...
- `POST /api/workflow-runs/{run_id}/resume`：只恢复 `paused_technical`，从已保存 operation
  继续；工作区越界和重复 Git 冲突必须使用干净重建，其他状态返回 `409 Conflict`
- `POST /api/workflow-runs/{run_id}/restart-clean`：只接受尚未开始远端发布的
  `paused_technical`。原子废弃旧 Run，从当前干净项目 HEAD 复用原 ChangeSpec、DesignNotes、
  WorkItems 和依赖创建唯一 replacement，不重新调用 Planner，也不迁移污染 diff；重复请求返回
  同一个 replacement
...
每轮需求 Prompt 都包含精简 `RequirementEvidenceIndex`；复用 session 时也不省略。工具提交后
后端立即校验引用并允许同 session 一次短修正，未通过的规格不会启动 Planner。Requirement
响应可选返回 `failure_stage` 与 `failure_code`，其中规格证据错误使用
`failure_stage=change_spec_validation`。
...
执行器按依赖 frontier 租约工作项。同层只有 2–3 项使用相同非空 `group`、每项具有非空且
两两不重叠的 `scope_hints` 时才允许并行，否则 Planner 必须生成串行依赖。并行项各自使用
独立分支、worktree 和 Pi session，成功后按 `position` cherry-pick 到 integration；实际路径
重叠时只允许一次 `superseded` 串行重跑，不计语义失败；单项 cherry-pick 失败、第二次重叠或
目标 worktree 脏污直接技术暂停。每个工作项
依次执行低档首次实现、低档普通修复和高档修复；结构化结果缺失或不合法同样只允许一次
同 session 短纠正。每次 attempt 都是独立持久 session，前后复核 HEAD、分支 ref 与 staged
指纹；`actual_attempt_count` 始终由真实 attempt 历史计算，`superseded` 与技术失败的 session、
结果和 usage 仍计入 Token 汇总。任务 Agent 可运行 Git 只读命令，受管
`raccoon:task-runtime:v4` extension 在工具调用前拒绝 Git 写命令，并将 read/edit/write/
grep/find/ls 与 shell 文件操作限制在当前 Pi cwd。调度器负责 stage、checkpoint commit 和最终集成。
每批 Agent 前后还会验证 integration 的 HEAD、分支、索引、未暂存、未跟踪和 Git operation
状态；越界变化立即记录 `workspace_violation` 并暂停，不再启动额外模型调用。进入 Prompt 的
验证摘要会移除 ANSI 和受管绝对路径，统一显示为 `<workspace>`。
...
WorkflowRun 通过最终 checkpoint 后进入 `publishing`：

- `local`：主工作区仍处于原始 HEAD 且干净时执行 fast-forward-only；
- `pull_request + GitHub`：推送唯一 workflow 分支，复用或创建 PR，以冻结 head SHA 启用
  merge commit 自动合并；
- `pull_request + GitLab`：使用 `RACCOON_GITLAB_TOKEN`（其次 `GITLAB_TOKEN`）调用 REST，
  复用或创建 MR，并携带冻结 SHA、`auto_merge=true` 和删除源分支设置；token 不进入 API、
  SQLite、事件或日志。

发布阶段按 `prepared → pushed → review_open → waiting_checks → merged → cleaning → completed`
持久化，恢复不会重复推送、建 PR/MR 或合并。远端 merge commit 是 PR/MR 模式的
`final_commit`；远端已合并后，本地主分支只有在仍处于原基线、工作区干净且可快进时才同步，
否则保存 `local_sync_skipped` 告警但 Run 仍可完成。远端 CI 明确失败时最多运行一次高档
`remote_ci_fix`，再失败或需要外部审批时进入 `paused_technical`，不消耗 Rescue。

成功完成前会清理 item/integration worktree、本地 workflow/item 分支和远端源分支；清理失败
暂停在可恢复的 `publication_cleanup`。`paused_technical`、`blocked`、`cancelled` 保留诊断现场。
Workflow 快照新增安全的 `publication` 和 `item_workspaces` 投影，item 绝对路径不会序列化。
...
`{ "theme_pack": "matcha" }` 独立持久化，不触发发布条件检查。`pull_request` 会创建并自动
合并远端 PR/MR，不会先移动本地主分支；每个 WorkflowRun 启动时冻结模式、provider、origin
和目标分支，之后修改设置只影响新 Run。提交模式显式保存后生效，只有模式实际变化时才重新
检查发布条件。host 仅支持
