...
| `notifications` | 通知生命周期、优先级、来源节点和 GrayDango 投影           |
...
监听、日志、Provider 非敏感字段、模型角色、验证命令、用量阈值、发布偏好、GitLab 主机、终端限制、网络策略（默认 `offline`，`package_registry`、`git_remote`、`readonly_fetch` 三档可选策略）和外观默认值都是事件化业务设置，投影到 `state.json`。CLI 和环境变量只用于启动覆盖；凭据不得进入事件或快照。
...
`RequirementSpec`、`RequirementRevision`、`WorkPlan`、`ProviderCapability`、`UsageSummary`、`EventEnvelope`、`StateFile` 和 `Notification` 是必须稳定的公共类型。
...
RequirementRevision {
  ...
  created_at, source_graph_id, source_branch_id, source_node_ids[], confirmation
}
...
- `Notification.source_workbench` 取值固定为 `conversation | delivery | files | git | terminal | models | settings | system`。来源为 `conversation` 时，定位语义是聚焦中央对话图对应分支节点（不打开工作台）；来源节点已被压缩折叠时先展开最近 checkpoint/详情节点；来源不存在时打开所属工作台诊断节点并解释。
- 对话图实体：单项目唯一 project graph，`graph_id` 固定；`RequirementRevision.source_graph_id` 与 `ConversationNode.graph_id` 字段保留用于事件聚合寻址与未来兼容。实现上可固定单值，并在类型注释中说明。
...
- `UsageSummary` 对未知 token 或价格保留为空，不用零代替。
...
- **BE-SPEC-002**（PRD-SPEC-005～006）：保存和确认使用 optimistic revision；冲突返回当前 revision 和字段差异。
...
- **BE-RUN-002**（PRD-RUN-002～003）：Planner 输出行为切片、依赖、场景引用、范围提示和验证目标。
...
- **BE-NOTIFY-002**：`notification.raised` 创建 active 通知，包含 `source_workbench` 与 `source_node_id`。
...
- **BE-API-004**（PRD-CANVAS-008）：危险命令采用 prepare/confirm 两阶段契约。prepare 返回动作摘要、目标、影响和绑定来源节点的短期确认 token；confirm 必须回传该 token 与幂等键，过期、目标变化或来源不匹配时拒绝执行。
...
其他危险命令（redact、强制发布、终端关闭运行中会话、发布重试）复用与 `POST /api/v1/git/actions/prepare` + `POST /api/v1/git/actions/confirm` 相同的 prepare/confirm 两阶段模式与 `…/prepare` + `…/confirm` 路由形态（BE-API-004）。计划编辑走既有 `PUT /api/v1/runs/{id}/plan`，不单设 pause-edits 端点。

`POST /api/v1/requirements/{id}/clarifications` 请求体携带问题节点 ID、选中选项与自定义文本；澄清回答作为 ClarificationAnswer 节点沿来源分支生成。
...
- **BE-GIT-002**：stage、unstage、commit、fetch、pull、push、switch、create 是唯一 v1 Git 写动作；危险动作要求确认节点产生的确认 token。
...
- **BE-TEST-NOTIFY-001**（AC-17）：通知优先级、去重、acknowledged/resolved、重启恢复和来源节点字段完整。
...
- **BE-E2E-004**（AC-08～09）：角色能力和软阈值行为正确。
...
| `BE-CHAT-*`                          | PRD-CHAT-001～014、AC-02、AC-12～13               |
| `BE-SPEC-*`                          | PRD-SPEC-001～008、AC-01、AC-13                   |
| `BE-TOOL-*`、`BE-NET-*`              | PRD-PUB-005、PRD-NFR-002～004                     |
| `BE-RUN-*`                           | PRD-RUN-001～009、PRD-EVENT-005、AC-02～03、AC-07 |
