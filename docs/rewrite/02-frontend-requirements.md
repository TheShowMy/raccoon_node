...
- **FE-BUILD-003**：假数据验收使用 `src/api/mock/` 和暂时的手写前端契约；演示控制台默认不挂载，只有显式设置 `VITE_ENABLE_DEMO_CONSOLE=true` 才允许在开发环境显示。后端接入时再以 Rust 生成契约替换手写类型，本轮不得伪造或修改后端 OpenAPI/JSON Schema 产物。
...
- **FE-GIT-002**：中栏按 staged、unstaged、untracked、conflicted 分组，选择后更新右栏 Diff；stage/unstage 即时执行，提交编辑区限制默认高度。文件行固定单行，正常密度约 `26px`、紧凑约 `22px`；状态和省略路径始终可见，操作按钮绝对定位在右侧，仅 hover、focus 或选中时出现且不撑高行，当前高度稳定显示至少 12–15 项。
...
- **FE-USAGE-002**：中部使用 CSS Grid 展示最近 365 天每日 Token 点阵，7 行、月份标记和 5 档色阶；每个日期可 hover 且可键盘聚焦读取日期与 Token。
