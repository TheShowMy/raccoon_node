...
前端把自动交付系统呈现为可理解、可控制的单项目节点空间。首屏是一个完整 React Flow 场景：中央对话列表（节点外观的垂直对话流）占据主要空间，初始只有一个 Composer；需求、文件、Git、终端、用量统计、设置节点固定环绕。模型配置并入设置；所有工作台仍以画布节点形式在射线外侧展开，不切换成传统业务页面。
...
- `@xyflow/react` 承载外层能力场景和需求交付全铺满子画布；中央对话是节点外观的垂直列表（v1 决策，见 §5），不使用嵌套画布。
...
| 节点       | 概览摘要                          | 打开后的节点关系                                      |
| ---------- | --------------------------------- | ----------------------------------------------------- |
| 中央对话列表 | 活动分支、流式节点、Composer      | 用户、过程、工具、回答、澄清、规格、确认              |
...
- **FE-CANVAS-001**（PRD-CANVAS-001～003）：首屏只挂载中央对话列表可视窗口邻近行和六个环绕能力节点。
...
- **FE-CANVAS-018**（PRD-CANVAS-007）：层级、依赖、来源和生命周期在需求交付画布中使用节点与边，在对话列表中使用节点行与脊椎线；普通工作台按领域布局组织 pane，输入、开关、下拉、文本编辑器和终端留在来源区域内。
...
> v1 决策（用户确认）：中央对话是**节点外观的垂直对话列表**，不是嵌套 React Flow 画布。
> 条目保留全部节点样式与组件（用户、过程、工具、回答、澄清、规格、确认、Composer、
> 过程组、操作确认/结果），行间以像素脊椎线保持节点图观感；滚动、文本选择、
> 长文滚动均为浏览器原生行为。下列条款中"对话图/节点图"均指该列表。

中央对话列表默认可直接操作，不存在“概览后再进入聊天”的第二步。节点链为：
...
- **FE-CHAT-011**（PRD-CHAT-009）：历史用户节点提供“从这里分支”；新分支共享祖先，原分支不可改写；列表形态下同一时刻展示活动分支链（共享祖先内联于链中），分支经页签切换，页签显示来源摘要。
- **FE-CHAT-012**：用户向上滚动或定位历史节点后暂停自动跟随，通过“回到最新”恢复；节点内部滚动不得误触暂停。发送、提交澄清、保存规格、确认或取消属于前向操作并恢复跟随；暂停期间流式增量不抢焦点。
- **FE-CHAT-013**：~~远景只显示节点类型、阶段和分支摘要，近景才渲染正文、工具摘要和操作。~~ v1 移除：列表形态无缩放与语义缩放，所有行渲染正文。
- **FE-CHAT-014**：重连先加载包含活动节点已组装内容的快照，再按全局 sequence 和 node sequence 对账，不产生重复节点或文本。
- **FE-CHAT-015**（PRD-CHAT-014）：~~任意对话节点提供 redact 入口~~ **v1 决策（用户确认）：不提供单节点 redact 入口**——对话节点不可逐条删除，新会话即可满足重新开始；redact 契约与确认链实现保留在 reducer/事件层，UI 入口后置。节点被 redact 时内容显示为“已删除”标记；节点 ID、结构、分支关系与相邻节点保持不变，附件引用显示已失效。
- **FE-CHAT-016**：列表为全宽行、节点列居中（节点最大宽度 720px），相邻行距恒为 `12px`；Composer、过程组和展开成员使用同一行模型，不使用另一套布局槽位。
- **FE-CHAT-017**：首次测量前按节点类型估算行高并缓存实测值；流式节点增高或过程组展开/折叠只影响其后行的偏移与总高，已完成行的位置与用户滚动位置保持稳定。
- **FE-CHAT-018**：自动跟随按“最后一个 `streaming | running` 节点 → 当前分支待操作澄清/确认节点 → 本次前向操作中新建的持久节点 → Composer”选择目标；跟随状态下把目标行贴底滚动到列表可视区底部安全边距内。同一批次产生澄清回答、规格和确认时只定位最后的可操作确认节点。
- **FE-CHAT-019**：跟随滚动使用平滑过渡且合并请求（后请求覆盖前请求），偏移小于 `2px` 不重复滚动；流式同一目标持续贴底；reduced-motion 下直接定位，不允许二次跳变。
- **FE-CHAT-020**：用户在对话列表向上滚动后暂停跟随并显示“回到最新”；点击该操作或切换分支恢复跟随；节点内部长文滚动不暂停。深链或 GrayDango 定位历史节点时保持暂停，不抢回末端。
...
- **FE-SPEC-001**（PRD-SPEC-004）：澄清问题、用户回答、规格 revision 和需求确认只注册在中央对话列表；需求工作台不得复制这些节点或提供编辑入口。
...
- **FE-PET-007**（PRD-NOTIFY-005）：`conversation` 通知聚焦中央对话列表对应分支节点；`delivery` 通知按射线流程打开需求工作台并聚焦 `source_node_id`；其余普通工作台通知只打开对应工作台，即使携带 `source_node_id` 也不定位内部 pane。
...
- 对话 DAG、分支、Composer 固化、分支输入门控、单选/多选/自由文本澄清、node delta、过程组和滚动窗口虚拟化。
- 不同高度节点、流式增高、过程组展开/折叠、分支与 Composer 的行距恒为 12px。
- 活跃行跟随贴底、手动暂停、恢复跟随和 reduced-motion。
...
- **FE-E2E-012**：连续多轮消息行距恒为 `12px`，跟随状态下活动生成节点和待操作业务节点保持列表可视区内贴底；节点内部滚动不暂停，真实向上滚动后自动跟随暂停，前向操作可恢复。
