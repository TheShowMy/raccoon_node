...
Raccoon Node 只服务当前一个 Git 仓库。现有“环绕节点 → 射线外侧工作台 → 相机聚焦 → 关闭恢复”具有明确的空间辨识度，必须保留。对话与需求适合用节点表达关系；文件、Git、终端、用量统计和设置更依赖紧凑浏览、表单与连续滚动，不应为了视觉一致强制使用内部画布。模型配置属于设置，不再与累计用量共用工作台。

### Decision

- 首页只有全屏外层 React Flow 场景和 GrayDango；不设置固定顶层业务区域。
- 中央是面积最大的独立 `ConversationGraph`，六个紧凑能力节点固定环绕。一个会话对应一张独立 graph；分支只在当前 graph 内共享祖先。
- 外层相机使用有界视差；能力节点不可拖动，中央对话使用独立滚动容器和窗口化列表。
- 初始对话图只有 Composer；发送后生成用户、过程、工具和回答并流式更新。普通问答恢复 Composer；开发目标从澄清到确认/取消期间由当前业务节点独占分支输入，不同时显示通用 Composer。
- 历史对话是不可变 DAG；澄清显式区分单选、多选和自由文本，规格 revision 和确认沿同一分支生成节点；完成过程可逆聚合。同一分支最多一个未完成引导式需求。
- 对话列表只使用一个可取消的滚动控制器，按“活动生成节点 → 待操作业务节点 → 本次新持久节点 → Composer”跟随，把目标行贴近可视区底部安全边距。新目标等待实测行高后平滑定位，晚到尺寸和流式增高做合并补偿。用户向上浏览和历史定位暂停跟随；节点内部滚动不暂停，前向业务操作恢复跟随。
- 对话列表右上角固定提供“＋ 新建会话”。空闲时直接创建独立 session；活动响应、草稿或输入门控存在时从当前输入所有者连接确认节点，确认后停止旧响应再切换。每个 `session_id + branch_id` 独立保存滚动位置、草稿、选择和过程组；旧 session 持久化但 v1 不提供历史会话 UI。
- 点击环绕节点时，以中心到触发节点的射线在触发节点外侧创建大型 `CanvasWorkbenchNode`，相机聚焦该节点。
- 同时只打开一个工作台；工作台只保留最小标题和关闭控制，其他节点作为低对比上下文保留。
- 关闭、Escape 和浏览器返回使用同一导航状态机，精确恢复外层 viewport、焦点、视差、对话与普通 pane 滚动位置，以及需求子画布 viewport。
- 需求交付是唯一全画布工作台；其余五个工作台使用领域化连续工具页面。节点视觉只用于主要 pane 的切角边框：正常密度间距 `2px`，紧凑密度和窄工作台 `1px`；不把内部行、字段或操作递归卡片化。
- Git 固化为仓库/变更/Diff 三栏，文件为资源管理器/预览双栏，终端为会话标签/单一活动终端；设置只保留通用、模型、运行与安全、维护四类，模型分类内部使用 Provider/模型/能力与角色三栏；用量统计独立展示 Token 指标、365 日点阵和模型聚合。窄工作台使用内部标签切换 pane。
- 危险操作在画布中从来源节点连接到 `ActionConfirmationNode`；普通工作台使用底部中央绝对定位的 `WorkbenchActionDock`，不改变 pane 几何或滚动。Dock 同时处理一个前台确认，按创建时间排队，完成结果 3 秒后收起；契约与画布确认节点相同，执行后形成结果事实。
- Git 变更行使用高密度单行布局：正常约 `26px`、紧凑约 `22px`；操作绝对定位在行右侧，只在 hover、focus 或选中时显示，不能撑高行。
- 默认任务预算位于设置的“运行与安全”，需求确认时可覆盖，Run 创建后冻结。累计用量工作台不显示预算或预算进度；Run 达到冻结预算 80% 只通过 `delivery` 告警。
- 澄清、回答、规格 revision 和需求确认全部在中央对话列表完成；需求工作台只接收已经确认或曾经执行的需求，按执行队列、进行中、阻断和历史投影，不提供返回、复制或编辑规格的入口。
- 需求工作台采用最大可用工作台节点，内部 React Flow 铺满内容区。主链固定为“确定需求 / Run → WorkPlan → 分层任务 → Integration Diff → 验证 → 审核 → 发布”。
- WorkPlan 只连接没有 `depends_on` 的根任务；每个依赖事实只生成一条边。任务以最长依赖路径定层，同层按父节点重心和稳定 position 排序；并行项纵向排列，显式合并任务独占下一层。终端任务之后只有一条质量主链。
- 普通任务固定 `360×208`，合并任务固定 `320×160`，同层边缘间距 `48px`、层间边缘距离至少 `112px`。状态、attempt 和节点内页签不影响坐标；分离端口与正交通道减少 fan-out/fan-in 和跳层边交叉。
- 计划确定性校验失败时只显示 `PlanInvalidNode` 与问题清单，不绘制正常依赖图。选择需求后仅对当前执行流水线做一次测量后的安全相机适配，后续状态变化不抢夺用户视口。
- 大输出使用详情节点、折叠节点或 artifact 引用；中央对话使用列表窗口化，需求画布使用 viewport 虚拟化控制规模。
- 基础输入、选择、提示、编辑器和终端控件可以位于节点内部，但不能承担全局导航或业务状态覆盖。

### Why

外层固定空间确保能力入口稳定，中央图承载无限增长与分支对话，射线工作台延续项目识别度；需求子画布保留复杂关系，其他工作台采用用户熟悉的领域工具布局，节点式 pane 边界只维持产品视觉连续性。
...
- **在需求工作台复制澄清、规格和确认节点**：产生两套编辑入口与事实边界；这些交互只属于中央对话，交付画布从确认事实开始。
- **WorkPlan 同时连接所有任务并叠加 `depends_on`**：制造重复边和错误父子语义；只连接根任务并保留真实依赖。
- **固定详情侧区**：形成第二套页面导航并压缩画布。
- **用全局覆盖确认危险操作**：失去来源关系和可追踪结果。
- **破坏性合并过程/工具节点**：丢失审计与恢复事实。
- **全部工作台画布化**：文件、Git、终端、用量统计、设置的浏览与表单密集操作在自由画布上不如领域化连续工具页直观（v2 假数据验收反馈）。
- **`auto-fit + minmax()` 卡片拼板**：响应式容易实现，但破坏 Git/文件等领域工具的主从层级，并形成大量等权小卡片。
- **无间距连续边框**：主区会视觉合并；保留 `1–2px` 极窄间距以识别独立 pane。

> 变更说明（2026-07-18）：文件、Git、终端、用量统计、设置收口为领域化连续工具页面；模型配置并入设置，危险操作使用不影响布局的底部悬浮 Dock，Git 使用高密度变更行。节点式视觉仅保留在主要 pane 边界，需求交付保持唯一全画布工作台。
...
- 气泡支持前后浏览、确认和来源定位；对话与需求来源可进一步聚焦 `source_node_id`，文件、Git、终端、用量统计、设置来源只按 ADR-011 打开工作台。模型配置问题打开设置，用量信息打开用量统计，任务预算告警打开需求工作台并聚焦 Run。
...
- 节点或普通 pane 内的持续状态不重复进入气泡；长错误和日志通过来源节点、功能区或 artifact 展示。
...
- 完成中央对话 DAG、流式过程/工具/回答、独立会话、会话内分支、平滑相机、节点化澄清、规格 revision 和确认。
- 完成环绕布局、射线工作台、视口恢复、GrayDango 队列、NDJSON 对账和重新同步。
- 交付标准：AC-10、AC-12～17、AC-19 中相关场景通过。
...
- 完成 Git 高密度三栏、文件双栏、终端标签页、独立用量统计和设置四分类（含模型三栏），以及大规模、可访问性和三平台打包。
- 交付标准：产品 AC-01 至 AC-19 都有可复现证据。
