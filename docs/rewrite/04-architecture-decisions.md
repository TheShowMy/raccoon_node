...
- 内层只使用一个可取消的相机控制器，按“活动生成节点 → 待操作业务节点 → 本次新持久节点 → Composer”跟随，把目标真实中心锚定到 50%/65% 并保持 zoom。新目标最多等待两个动画帧取得实测尺寸，以 220ms ease-out 平滑定位；晚到尺寸和流式增高用 100ms 平滑补偿，禁止 0ms 闪动校正。只有真实画布浏览和历史定位暂停；节点内部滚动不暂停，前向业务操作恢复跟随。
- 对话图右上角固定提供“＋ 新建会话”。空闲时直接创建独立 session；活动响应、草稿或输入门控存在时从当前输入所有者连接确认节点，确认后停止旧响应再切换。每个 `session_id + branch_id` 独立保存 viewport、草稿、选择和过程组；旧 session 持久化但 v1 不提供历史会话 UI。
...
- 澄清、回答、规格 revision 和需求确认全部在中央对话图完成；需求工作台只接收已经确认或曾经执行的需求，按执行队列、进行中、阻断和历史投影。确定需求摘要可返回对话确认节点，但工作台不复制或编辑规格。
