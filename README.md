...
点击项目画布左上角的“设置”节点会打开右侧设置抽屉。主题和提交模式保存后立即
生效；监听地址或端口变更会通过 Web 重启服务。选择 `0.0.0.0` 时必须确认无鉴权
API 暴露风险。模型页可打开 Pi 登录终端，用户手动输入 `/login` 后再重载 Pi RPC
模型列表；Raccoon 不读写 Pi 的认证文件或 `models.json`。
