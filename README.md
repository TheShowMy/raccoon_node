...
└── .raccoon-node/
    ...
    ├── worktrees/
    ├── attachments/
    └── app.json.migrated  # 仅旧版数据迁移后存在
```

`data.db` 是唯一业务主存储；Pi session 保存完整模型上下文，日志按日滚动并保留
最近 7 个文件，`extensions/` 仅存放程序内置的受管 Pi extension。首次启动新版时，
旧 `app.json` 会在单个事务中导入 SQLite，成功后原子改名为
`app.json.migrated`；迁移备份不会自动删除。后续启动不再读取该备份。
