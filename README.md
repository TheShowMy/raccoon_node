...
- 统一对话：项目问答与需求澄清共用消息时间线，实时展示回答、思考和工具状态
- 项目问答：基于当前仓库维护连续的只读会话，并可用 `/生成需求说明` 汇总当前问答
- 需求澄清：可接收问答生成的需求说明预填，分析需求、提出澄清问题并生成确认草案
...
项目问答输入 `/` 可选择“生成需求说明”。生成结果会独立保存在问答会话末尾；
点击“作为需求继续”只会切换到需求澄清并预填输入框，不会自动创建或发送需求，
也不会迁移问答附件；用户确认、修改后再提交。项目问答与需求澄清使用独立 Pi
session，切换不会清空问答历史。
