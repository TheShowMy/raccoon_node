...
`/需求生成 <描述>`。主聊天已有 Pi session 时，后端通过 Pi RPC `clone` 创建需求
分支并立即把客户端切回主 session；主聊天为空时直接创建独立需求 session。需求
分支保留自己的消息、附件、思考、工具、停止和错误状态，活动期间不能新建主会话。

确认需求后，需求立即进入 FIFO/DAG 执行，中心节点返回项目聊天；后端异步把最终
需求摘要写入主 Pi session。聊天中只显示结构化系统摘要卡片，不显示 Pi 的内部确认
回复。写回失败不回滚需求，卡片会显示原因并提供重试；写回期间沿用项目聊天
single-flight，普通消息和会话切换返回冲突。取消或放弃需求不会写回摘要。
...
`data.db` 是唯一业务主存储，包含项目问答、需求说明、需求对话快照、FIFO、DAG
和恢复状态；Pi session 只保存完整模型上下文。对话 WebSocket 仅推送实时增量，
