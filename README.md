...
点击项目画布左上角的“设置”节点会向左上展开设置工作台。主题点击后立即切换并
持久化；提交模式、监听地址和端口使用显式保存，监听变更会通过 Web 重启服务。
选择 `0.0.0.0` 时必须在节点内确认无鉴权 API 暴露风险。模型页可打开 Pi 登录
终端，用户手动输入 `/login` 后再重载 Pi RPC 模型列表；Raccoon 不读写 Pi 的
认证文件或 `models.json`。
