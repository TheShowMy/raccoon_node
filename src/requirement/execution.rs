...
- 禁止手动暂存或提交代码；Git 暂存、提交和后续集成由外部调度程序统一处理。
- 允许只读 Git 命令，例如 git status、git diff、git diff --cached、git log。"#;
...
mod tests {
    ...
    fn implementation_prompt_does_not_contain_full_draft() {
        ...
        assert!(
            ...
            "draft section must not appear for implementation tasks"
        );
        assert!(
    ...
    fn implementation_prompt_shows_only_direct_dependencies() {
        ...
        assert!(
            ...
            "completed tasks section must not appear for implementation tasks"
        );
        assert!(
            !prompt.contains("任务 B"),
            "direct dependency output must not appear"
        );
        assert!(
    ...
    #[test]
    fn implementation_prompt_omits_draft_and_completed_sections() {
        let mut requirement = test_requirement("实现页面");
        if let Some(ref mut draft) = requirement.draft {
            draft.title = "完整需求标题".to_owned();
            draft.summary = "完整需求摘要".to_owned();
        }
        let mut completed_b = test_task("task-b", "任务 B", Vec::new());
        completed_b.status = RequirementTaskStatus::Completed;
        completed_b.result_summary = Some("完成了 B".to_owned());
        let current = test_task("task-c", "任务 C", vec!["task-b".to_owned()]);
        let plan = RequirementExecutionPlan {
        ...
        assert!(
            !prompt.contains("## 确认需求草案"),
            "draft section must be omitted"
        );
        assert!(
            !prompt.contains("## 已完成前置任务"),
            "completed tasks section must be omitted"
        );
    }
    ...
    fn merge_review_prompt_forbids_publication_commands() {
        ...
        assert!(
            prompt.contains("## 确认需求草案"),
            "MergeReview must still see the draft section"
        );
        assert!(
            prompt.contains("## 已完成前置任务"),
            "MergeReview must still see the completed tasks section"
        );
        assert!(prompt.contains("标题：最终审核"));
        assert!(prompt.contains("摘要：最终审核"));
    }

    #[test]
    fn branch_merge_prompt_keeps_context_sections() {
        let requirement = test_requirement("最终审核");
        ...
        assert!(
            prompt.contains("## 确认需求草案"),
            "BranchMerge must still see the draft section"
        );
        assert!(
            prompt.contains("## 已完成前置任务"),
            "BranchMerge must still see the completed tasks section"
        );
    }
