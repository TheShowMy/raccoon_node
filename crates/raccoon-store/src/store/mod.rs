...
use raccoon_core::file_refs::{build_prompt_images, build_reference_context};
...
impl JsonStore {
        ...
        for requirement in &mut data.requirements {
            ...
            if let Some(plan) = requirement.execution_plan.as_mut() {
                for task in &mut plan.tasks {
                    ...
                    {
                        raccoon_core::models::RequirementModelTier::High
                    } else {
    ...
    pub async fn apply_requirement_clarification_request(
        &mut self,
        requirement_id: &str,
        clarifications: Vec<raccoon_core::models::RequirementClarification>,
    ) -> Result<(), AppError> {
    ...
    pub async fn recover_task_group(
        ...
        let in_group = |task: &raccoon_core::models::RequirementExecutionTask| {
