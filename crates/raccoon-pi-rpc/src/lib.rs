...
use raccoon_core::utils::{ensure_child_path, normalize_local_path, resolve_git_root};
use raccoon_requirement::{
...
impl PiRpcModelProvider {
    ...
    async fn project_client(
        &self,
        project: &raccoon_core::models::Project,
...
impl PiRpcClient {
    ...
    pub async fn analyze_requirement(
        ...
        let trace = self
            .attach_session_usage(
                raccoon_requirement::build_pi_trace_metadata(&pi_events),
                session_reused,
            )
            .await;
    ...
    pub async fn plan_requirement_execution(
        ...
        input: RequirementPlanInput,
        events: Option<RequirementEventEmitter>,
    ) -> Result<raccoon_core::models::RequirementExecutionPlan, AppError> {
    ...
    pub async fn execute_requirement_task(
        ...
        let session_reused = self
            ...
            .await?;
        if input.task.recovery_stage == raccoon_core::models::RequirementRecoveryStage::GuidedRetry
    ...
    pub async fn ask_project_chat(
        ...
        self.prompt_with_images(
            &raccoon_chat::build_project_chat_prompt(&input, session_reused),
            &input.prompt_images,
        )
        .await?;
        let mut pi_events = Vec::new();
    ...
    async fn extract_pi_response(
        &self,
        events: &[Value],
    ) -> Result<raccoon_requirement::PiResponseExtraction, PiResponseFailure> {
        let last_assistant_text =
            self.get_last_assistant_text()
                .await
                .map_err(|error| PiResponseFailure {
                    message: error.to_string(),
                    trace: raccoon_requirement::build_pi_trace_metadata(events),
                })?;
...
fn dependency_branches_for_task(
    task: &raccoon_core::models::RequirementExecutionTask,
    plan: &raccoon_core::models::RequirementExecutionPlan,
) -> Vec<String> {
    match task.kind {
        RequirementTaskKind::Implementation
        | RequirementTaskKind::BranchMerge
...
pub(crate) async fn stage_task_changes(
    ...
    if task.status == raccoon_core::models::RequirementTaskStatus::Fixing {
