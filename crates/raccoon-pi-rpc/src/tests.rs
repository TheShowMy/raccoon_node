...
fn pi_working_directory_is_limited_to_project_root_or_managed_worktree() {
    ...
    assert_eq!(
        ...
        raccoon_core::utils::normalize_local_path(&std::fs::canonicalize(&project).unwrap())
            .unwrap()
    );
...
fn test_task(path: &Path) -> raccoon_core::models::RequirementExecutionTask {
    raccoon_core::models::RequirementExecutionTask {
        id: "task-1".to_owned(),
        title: "实现功能".to_owned(),
        ...
        recovery_stage: raccoon_core::models::RequirementRecoveryStage::None,
